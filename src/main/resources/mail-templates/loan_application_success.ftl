      ...
      <div class="content" style="background-color:#ffffff;color:#090909">
        ...
        <div class="message" style="padding:30px;text-align:justify;line-height:25px">
          <p style="Margin-Top:0;font-size:20px">
            Kính gửi Quý Khách hàng <#if name??>${name?html}</#if>,
          </p>
          <p>Chúc mừng Quý Khách hàng đã nộp hồ sơ vay
            mã <#if loanApplication.loanCode??>${loanApplication.loanCode}</#if> thành công tại Ngân
            hàng MSB! lúc <#if successTime??>${successTime}</#if></p>
