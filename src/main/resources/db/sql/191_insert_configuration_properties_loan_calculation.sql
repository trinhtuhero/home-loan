delete from properties where name = 'loan-calculation';
INSERT INTO properties (uuid, created_at, updated_at, name, description, current_value, default_value) VALUES ('0e5a8854-fbe5-4428-977b-04ff40e18dcb', '2023-03-31 02:36:35.789979', '2023-03-31 03:20:45.707443', 'loan-calculation', null, '{
  "data": {
    "income": {
      "rules": [
        {
          "type": "min",
          "value": "5000000",
          "message-error": "Tổng thu nhập bạn cần có ít nhất là 5 triệu"
        },
        {
          "required": true
        }
      ]
    },
    "monthly_payment_amount": {
      "formula": "value = 0.7*income-2000000",
      "rules": [
        {
          "type": "validate",
          "condition": "monthly_payment_amount > value",
          "message_error": "Dư nợ vượt quá khả năng tài chính của bạn"
        },
        {
          "required": false
        }
      ]
    },
    "collateral_value": {
      "formula": "loan_amount/0.8",
      "rules": [
        {
          "value": "200000000",
          "type": "min",
          "message_error": "Giá trị TSBĐ tối thiểu là 200 triệu đồng"
        },
        {
          "required": false
        },
        {
          "value": "100000000000",
          "type": "max",
          "message_error": "Giá trị TSBĐ tối đa là 100 tỷ"
        }
      ]
    },
    "loan_time": {
      "formula": "0.7*income-monthly_payment_amount",
      "rule": [
        {
          "type": "case",
          "condition": "value >= 2_000_000 && value < 30_000_000",
          "value": 240
        },
        {
          "required": true
        },
        {
          "type": "case",
          "condition": "value >= 30_000_000",
          "value": 300
        },
        {
          "type": "min",
          "value": 3,
          "message_error": "Thời hạn vay tối thiểu là 3 tháng"
        },
        {
          "type": "max",
          "value": 300,
          "message_error": "Thời hạn vay tối đa là 300 tháng"
        }
      ]
    },
    "loan_amount": {
      "formula": "(365*(0.7*${income}-${monthly_payment_amount})*${loan_time})/(365+${loan_time}*(0.126*30))",
      "rules": [
        {
          "type": "min",
          "value": 100000000,
          "message_error": "Số tiền cần vay tối thiểu là 100 triệu"
        },
        {
          "required": false
        },
        {
          "type": "max",
          "value": 20000000000,
          "message_error": "Số tiền cần vay tối đa là 20 tỷ"
        },
        {
          "condition": "collateral_value*0.8 < value",
          "type": "validate",
          "message_error": "Số tiền cần vay tối đa không vượt quá 80% giá trị TSBĐ"
        }
      ]
    },
    "interest_rate": {
      "values": [
        {
          "name": "4,99% - 3 tháng đầu",
          "key": "IR499",
          "default": false
        },
        {
          "name": "6,99% - 6 tháng đầu",
          "key": "IR699",
          "default": false
        },
        {
          "name": "7,99% - 12 tháng đầu",
          "key": "IR799",
          "default": true
        },
        {
          "name": "8,75% - 18 tháng đầu",
          "key": "IR875",
          "default": false
        },
        {
          "name": "9,15% - 24 tháng đầu",
          "key": "IR915",
          "default": false
        }
      ]
    },
    "debt-method": {
      "values": [
        {
          "name": "Gốc trả đều hàng tháng, lãi giảm dần",
          "key": "GTDHH",
          "default": false
        },
        {
          "name": "Gốc trả đều 02 tháng/lần, lãi giảm dần",
          "key": "GTD2T",
          "default": false
        },
        {
          "name": "Gốc trả đều 03 tháng/lần, lãi giảm dần",
          "key": "GTD3T",
          "default": false
        },
        {
          "name": "Gốc tăng dần, lãi theo dư nợ thực tế",
          "key": "GTD",
          "default": false
        },
        {
          "name": "Gốc giảm dần, lãi theo dư nợ thực tế",
          "key": "GGD",
          "default": false
        }
      ]
    },
    "status": 200
  }
}', null);
